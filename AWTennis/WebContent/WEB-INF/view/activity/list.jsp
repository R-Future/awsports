		 	...
		 	<table id="activity_list" class="table table-striped table-bordered">
				...
				<c:if test="${ activities.size()>0 }">
					<c:forEach var="i" begin="0" end="${ activities.size()-1 }" step="1">
						...
						<td>
							<a href="<%=request.getContextPath()%>/activity/update?id=${activities[i].activity.id}" title="修改"><i class="fa fa-edit"></i></a>
							<a href="<%=request.getContextPath()%>/activity/delete?id=${activities[i].activity.id}" title="删除"><i class="fa fa-trash"></i></a>
						</td>
						</tr>
					</c:forEach>
				</c:if>
