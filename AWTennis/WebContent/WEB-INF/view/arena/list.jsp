		 	...
		 	<table id="arena_list" class="table table-striped table-bordered">
				...
				<c:if test="${arenas.size()>0}">
					<c:forEach var="i" begin="0" end="${arenas.size()-1}" step="1">
						...
						<td>${ arenas[i].note }</td>
						<td>
							<a href="<%=request.getContextPath()%>/arena/update?id=${arenas[i].id}" title="修改"><i class="fa fa-edit"></i></a>
							<a href="<%=request.getContextPath()%>/arena/delete?id=${arenas[i].id}" title="删除"><i class="fa fa-trash"></i></a>
						</td>
						</tr>
					</c:forEach>
				</c:if>
