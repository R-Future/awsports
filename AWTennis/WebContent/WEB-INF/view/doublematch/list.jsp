				...
				<div class="col-md-12">
					<div class="form-group">
						...
						<div class="input-group">
							...
							<form:input type="input" path="doublematch.startedat" id="startedAt" class="form-control date" data-date-format="yyyy-mm-dd"/>
						</div>
						-
						...
						<div class="input-group">
							...
							<form:input type="input" path="doublematch.endedat" id="endedAt" class="form-control date" data-date-format="yyyy-mm-dd"/>
						</div>
					</div>
		 	...
		 	<table id="doublematch_list" class="table table-striped table-bordered">
				...
				<c:if test="${ doublematchQuerys.size()>0 }">
					<c:forEach var="i" begin="0" end="${doublematchQuerys.size()-1}" step="1">
						...
						<td>${ doublematchQuerys[i].hcTeamQuery.user1.name }&${ doublematchQuerys[i].hcTeamQuery.user2.name } <b>VS</b> ${ doublematchQuerys[i].apTeamQuery.user1.name }&${ doublematchQuerys[i].apTeamQuery.user2.name }</td>
						<td>
						...
						<td>
							<a href="<%=request.getContextPath()%>/doublematch/update?id=${doublematchQuerys[i].doublematch.id}" title="修改" onclick="return updateConfirm();"><i class="fa fa-edit"></i></a>
							<a href="<%=request.getContextPath()%>/doublematch/delete?id=${doublematchQuerys[i].doublematch.id}" title="删除" onclick="return deleteConfirm();"><i class="fa fa-trash"></i></a>
						</td>
						</tr>
					</c:forEach>
				</c:if>
