		...
		<c:forEach var="i" begin="0" end="${ doublematchQuerys.size()-1 }" step="1">
			<tbody>
				<tr>
					...
					<td>
					<select name="doublematchQuerys[${i}].doublematch.homecontestant" class="" onclick="verifyPlayer(this)">
					...
					vs
					<select name="doublematchQuerys[${i}].doublematch.awayplayer" class="" onclick="verifyPlayer(this)">
						<option value="0">-请选择-</option>
						<c:forEach items="${ teamQuerys }" var="teamQuery">
							<option value="${ teamQuery.team.id }">${ teamQuery.team.name }</option>
						</c:forEach>
