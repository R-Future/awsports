		 	...
		 	<table id="level_list" class="table table-striped table-bordered">
				...
				<c:if test="${levels.size()>0}">
					<c:forEach  var="i" begin="0" end="${levels.size()-1}" step="1">
						...
						<td>${ levels[i].note }</td>
						<td>
							<a href="<%=request.getContextPath()%>/level/update?id=${levels[i].id}" title="修改" onclick="return updateConfirm();"><i class="fa fa-edit"></i></a>
							<a href="<%=request.getContextPath()%>/level/delete?id=${levels[i].id}" title="删除" onclick="return deleteConfirm();"><i class="fa fa-trash"></i></a>
						</td>
						</tr>
					</c:forEach>
				</c:if>
