		 	...
		 	<table id="punishment_list" class="table table-striped table-bordered">
				...
				<c:if test="${punishmentQuerys.size()>0}">
					<c:forEach var="i" begin="0" end="${punishmentQuerys.size()-1}" step="1">
						...
						<td>
							<a href="<%=request.getContextPath()%>/punishment/update?id=${punishmentQuerys[i].punishment.id}" title="修改" onclick="return updateConfirm();"><i class="fa fa-edit"></i></a>
							<a href="<%=request.getContextPath()%>/punishment/delete?id=${punishmentQuerys[i].punishment.id}" title="删除" onclick="return deleteConfirm();"><i class="fa fa-trash"></i></a>
						</td>
						</tr>
					</c:forEach>
				</c:if>
