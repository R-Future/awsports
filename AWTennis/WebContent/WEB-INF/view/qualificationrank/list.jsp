...
<div class="content-wrapper">
	...
	<div class="container-fluid">
		...
		<fieldset>
			...
			<a href="javascript:void(0)" id="updateQualificationRank" title="更新排名" class="btn btn-primary"><i class="fa fa-refresh"></i></a>
		 	...
		 	<table id="qualificationrank_list" class="table table-striped table-bordered">
				...
				<c:if test="${qualificationrankQuerys.size()>0}">
					<c:forEach var="i" begin="0" end="${qualificationrankQuerys.size()-1}" step="1">
						...
						<td><fmt:formatNumber type="percent" maxIntegerDigits="3" value="${ qualificationrankQuerys[i].qualificationrank.totalwins/qualificationrankQuerys[i].qualificationrank.totalmatchs }"/></td>
						...
						<td>
							<a href="<%=request.getContextPath()%>/qualificationrank/update?id=${qualificationrankQuerys[i].qualificationrank.id}" title="修改" onclick="return updateConfirm();"><i class="fa fa-edit"></i></a>
							<a href="<%=request.getContextPath()%>/qualificationrank/delete?id=${qualificationrankQuerys[i].qualificationrank.id}" title="删除" onclick="return deleteConfirm();"><i class="fa fa-trash"></i></a>
						</td>
						</tr>
					</c:forEach>
				</c:if>
...
$(function(){
	...
	//update rank
	$("#updateQualificationRank").click(function(){
		var loader = $("div.loader");
		var tournamentids=$("input[name='tournamentids']:checked").serialize();
		...
		$.ajax({
			...
			success:function(){
				...
				$("#updateQualificationRank").removeAttr("disabled");
				alert("更新成功！");
			}
		});
	});
	...
	$("#t2e").click(function(){
		$("#qualificationrank_list").table2excel({
			exclude: ".excludeOption",
			name: "qualificationRank",
			filename: "qualificationRank.xls",
			exclude_img: true,
			exclude_links: true,
			exclude_inputs: true
		});
