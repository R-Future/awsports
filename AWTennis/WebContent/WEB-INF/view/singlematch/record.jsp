		...
		<c:forEach var="i" begin="0" end="${ singlematchQuerys.size()-1 }" step="1">
			<tbody>
				<tr>
					...
					<td class="players">
					<select name="singlematchQuerys[${i}].singlematch.homecontestant" class="" onchange="verifyPlayer(this)">
					...
					vs
					<select name="singlematchQuerys[${i}].singlematch.awayplayer" class="" onchange="verifyPlayer(this)">
						<option value="0">-请选择-</option>
						<c:forEach items="${ userQuerys }" var="userQuery">
							<option value="${ userQuery.user.id }">${ userQuery.user.name }</option>
						</c:forEach>
