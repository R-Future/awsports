				 	...
				 	<table id="team_list" class="table table-striped table-bordered">
						...
						<c:if test="${ teams.size()>0 }">
							<c:forEach var="i" begin="0" end="${ teams.size()-1 }" step="1">
								...
								<td>
									<a href="<%=request.getContextPath()%>/team/update?id=${teams[i].team.id}" title="修改"><i class="fa fa-edit"></i></a>
									<a href="<%=request.getContextPath()%>/team/delete?id=${teams[i].team.id}" title="删除"><i class="fa fa-trash"></i></a>
								</td>
								</tr>
							</c:forEach>
						</c:if>
