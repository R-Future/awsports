				...
				<tbody>
					...
					<c:if test="${ pointrules.size()>0 }">
						<c:forEach var="i" begin="0" end="${ pointrules.size()-1 }" step="1">
							...
							<td>
								<a href="<%=request.getContextPath()%>/pointrule/update?id=${pointrules[i].id}" title="修改" onclick="return updateConfirm();"><i class="fa fa-edit"></i></a>
								<a href="<%=request.getContextPath()%>/pointrule/delete?id=${pointrules[i].id}&tournamentid=${tournamentQuery.tournament.id}" title="删除" onclick="return deleteConfirm();"><i class="fa fa-trash"></i></a>
							</td>
							</tr>
						</c:forEach>
					</c:if>
