			 	...
			 	<table id="tournament_list" class="table table-striped table-bordered">
					...
					<c:if test="${tournaments.size()>0}">
						<c:forEach var="i" begin="0" end="${ tournaments.size()-1 }" step="1">
							...
							<td>
								<a href="<%=request.getContextPath()%>/tournament/update?id=${tournaments[i].tournament.id}" title="修改"><i class="fa fa-edit"></i></a>
								<a href="<%=request.getContextPath()%>/tournament/delete?id=${tournaments[i].tournament.id}" title="删除"><i class="fa fa-trash"></i></a>
							</td>
							</tr>
						</c:forEach>
					</c:if>
