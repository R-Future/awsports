								...
								<c:if test="${ userQuerys.size()>0 }">
									<c:forEach var="i" begin="0" end="${ userQuerys.size()-1 }" step="1">
										<tr>
											...
											<td>
												<a href="<%=request.getContextPath()%>/user/update?id=${userQuerys[i].user.id}" title="修改" onclick="return updateConfirm();"><i class="fa fa-edit"></i></a>
												<a href="<%=request.getContextPath()%>/user/delete?id=${userQuerys[i].user.id}" title="删除" onclick="return deleteConfirm();"><i class="fa fa-trash"></i></a>
											</td>
										</tr>
									</c:forEach>
								</c:if>
