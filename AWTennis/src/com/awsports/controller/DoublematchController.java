...
public class DoublematchController {
	...
	public String list(Model model,DoublematchQuery doublematchQuery) throws Exception{
		//默认只查询当前赛季比赛记录
		if(doublematchQuery.getDoublematch() == null){
			doublematchQuery = new DoublematchQuery();
			Doublematch doublematch = new Doublematch();
			doublematch.setStartedat(Calendar.getInstance().get(Calendar.YEAR)+"-01-01");
			doublematch.setEndedat(new SimpleDateFormat("yyyy-MM-dd").format(Calendar.getInstance().getTime()));
