...
public class QualificationrankController {
	...
	//logger
	private Logger logger = Logger.getLogger(QualificationrankController.class);
	 ...
	 * 
	 * @Author: peRFect
	 * @Datetime: 2017年12月10日 下午3:49:22
	 * @param tournamentids
	 * @return
	 * @throws Exception
	 * @Return: String
	 * @Description: 按照赛事类型更新排名
	 *
	 */
	...
	public String updateRank(Integer[] tournamentids) throws Exception{
		if(tournamentids == null || tournamentids.length == 0){
			...
			updateRankCore(tournamentids);
			logger.info("["+Calendar.getInstance().getTime().toString()+"] 手动更新资格赛排名成功");
			return "forward:list";
		}
	}
	 ...
	 * 
	 * @Author: peRFect
	 * @Datetime: 2017年12月11日 下午10:08:06
	 * @param tournamentids
	 * @Return: void
	 * @Description: 更新排名
	 *
	 */
	private void updateRankCore(Integer[] tournamentids){
		
		String sqlError = "["+Calendar.getInstance().getTime().toString()+"] sql error, ";
		...
		for(Integer tournamentid:tournamentids){
			...
			try {
				...
				logger.error(sqlError+"failed to get individual points");
			}
					...
					if(ip.getInvalid().booleanValue()){//数据无效
						...
						if(ip.getUserid().intValue()==userid){//同一个用户
							...
							try {
								...
								// TODO Auto-generated catch block
								logger.error( sqlError + "failed to get old qualification rank");
								e.printStackTrace();
							}
							...
							Integer punishmentPoints;
							try {
								punishmentPoints = punishmentService.findSumByUseridTournamentidYear(punishment);
								...
								// TODO Auto-generated catch block
								logger.error(sqlError+"failed to get punishment points");
								e.printStackTrace();
							}
...
//					qualificationrank.setRankingchange(0);						
				...
				if(rawRanks.size()>0){
					try{
						...
						logger.error(sqlError+"failed to update qualification rank");
					}
				}else{
					//...
				}
