...
public class TeampointController {
	...
	public String list(Model model,TeampointQuery teampointQuery) throws Exception{
		//默认只显示当前年份的积分
		if(teampointQuery.getTeampoint() == null){
			teampointQuery = new TeampointQuery();
			Teampoint teampoint = new Teampoint();
			teampoint.setYear(Calendar.getInstance().get(Calendar.YEAR));
			teampointQuery.setTeampoint(teampoint);
		}
