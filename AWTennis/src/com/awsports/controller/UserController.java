...
public class UserController {
	...
	public String profile(Model model, Integer id) throws Exception{
		if(id==null){
			...
			model.addAttribute("singlematchs", ResponseInfo.getSinglematchInfo(singlematchQuerys));
			//个人排名最高纪录
			List<Individualrankest> individualrankests = individualrankestService.findByUser(user);
			model.addAttribute("individualrankests", individualrankests);
			//双打比赛记录
			List<DoublematchQuery> doublematchQuerys=doublematchService.findByUser(user);
			model.addAttribute("doublematchs", ResponseInfo.getDoublematchInfo(doublematchQuerys, id));
			//组合排名最高纪录
			Map<Team,List<Teamrankest>> teamrankestRecords = new HashMap<Team,List<Teamrankest>>();
			List<Team> teams = teamService.findByUser(user);
			for(Team team : teams){
				teamrankestRecords.put(team, teamrankestService.findByTeam(team)); 
			}
			model.addAttribute("teamrankestRecords", teamrankestRecords);
			model.addAttribute("sexTypes", sexTypes);
			model.addAttribute("forehandTypes", forehandTypes);
			model.addAttribute("backhandTypes", backhandTypes);
	...
	public void verifyUsername(String currentName, HttpServletResponse response) throws Exception{
		JSONArray jsonArray = new JSONArray();
		if(userService.findByName(currentName) == null){
